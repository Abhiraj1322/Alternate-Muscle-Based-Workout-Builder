...
const MyWorkouts = () => {
  ...
  const deleteWorkout = async (workoutId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:8000/workout/${workoutId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      {filteredWorkouts.length > 0 ? (
        <ul className="space-y-6">
          {filteredWorkouts.map(workout => (
            <li key={workout._id} className="bg-[#1c1c1e] rounded-lg p-4 shadow-lg border border-gray-700">
              <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4 mb-3">
                <div className="flex-1">
                  <h3 className="text-xl sm:text-2xl font-semibold text-blue-400">{workout.name}</h3>
                  <p className="text-sm text-gray-400">Day: {workout.day}</p>
                  {workout.notes && <p className="text-sm text-gray-300 italic mt-1">Note: {workout.notes}</p>}
                </div>
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {workout.exercises.map((ex, index) => (
                    <li key={index} className="bg-[#2a2a2e] p-3 rounded border border-gray-600">
                      <p><span className="font-semibold">Type:</span> {ex.type}</p>
